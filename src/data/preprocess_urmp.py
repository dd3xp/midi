...
用法:
  python preprocess_urmp.py                # 处理所有支持的乐器
  python preprocess_urmp.py vn             # 只处理小提琴
  python preprocess_urmp.py vn fl          # 处理小提琴和长笛
"""
...
from src.data.preprocess import (
    extract_amp, estimate_velocity, synthesize, hz_to_midi, notes_to_midi,
    HOP_TIME, TARGET_SR,
)

URMP_DIR = os.path.join(PROJECT_ROOT, "dataset", "solo", "URMP")
OUTPUT_BASE = os.path.join(PROJECT_ROOT, "datagen", "solo", "URMP")
...
def parse_notes(filepath):
    ...
    velocity 暂设为占位符 0，后续由 estimate_velocity() 根据音频 RMS 估算。
    """
    data = np.loadtxt(filepath)
    if data.ndim == 1:
    ...
    velocities = np.zeros_like(onsets)  # placeholder, filled by estimate_velocity()
