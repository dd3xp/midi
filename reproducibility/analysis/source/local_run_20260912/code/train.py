...
def make_model(name):
    config=({'d_model':128,'n_layers':6,'d_state':16,'d_conv':4,'expand':1,'dropout':.2,'bidirectional':True}
            if name=='mamba' else {'d_model':256,'n_layers':4,'d_state':64,'dropout':.2,'bidirectional':True})
    return BaselineModel(dropout=.3,encoder_input_dim=20,encoder_type=name,encoder_config=config,
                         amp_attention=True,amp_attn_heads=8,amp_attn_layers=2,amp_attn_ff=1024,
                         note_aware_amp_v2=True)
...
def main():
    ...
    parser.add_argument('--model',choices=['mamba','s4d'],required=True)
