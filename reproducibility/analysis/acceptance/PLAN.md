...
After the first main-PDF review, before computing any public direction counts: extend the same Full/Note/Error direction analysis to both already-frozen public-system RMS arrays, against instrument OLS and the shared envelope on their full declared populations (186 and 46). Also report the existing target-deduplicated and single-lane subsets. These are output diagnostics across training protocols, not fair architecture rankings, independent data, or new clean outer tests. No additional systems or checkpoint selection.
